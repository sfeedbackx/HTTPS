...
![HTTP Request Headers](./assets/http-request-headers.png)
...
An `HTTP` status code is a three-digit number that indicates the result of the `HTTP` request. Common status codes include:
...
![HTTP Response Headers](./assets/http-response-headers.png)
...
Keep in mind that `HTTP` is a "stateless" protocol, which means that each command runs independently
of any other command. In the original spec, `HTTP` requests each created and closed a TCP
connection. In newer versions of the HTTP protocol (HTTP 1.1 and above), persistent connections
allow for multiple HTTP requests to pass over a persistent [TCP](https://www.cloudflare.com/learning/ddos/glossary/tcp-ip/) connection, improving resource
